...
def calculate_final_price(price, value_float):
    if 0.005 <= value_float < 0.01:
        ...
        return price * 0.8
    elif 0.5 >= value_float > 0.15:
        return price * 0.6
    elif 1 > value_float > 0.5:
        return price * 0.4
    else:
        return price
    ...
    while True:
        ...
        if choice == "1" or choice.lower() == "alpha case":
            ...
            if balance >= case_price:
                ...
                item, rarity, item_price, condition, float_value = open_alpha_case(intrigue_enabled)
            ...
            if balance >= case_price:
                ...
                item, rarity, item_price, condition, float_value = open_beta_case(intrigue_enabled)
                collected_items.append((item, rarity, item_price, condition, float_value))
            else:
                print("Sorry,you don't have enough money")
            ...
            if balance >= case_price:
                ...
                item, rarity, item_price, condition, float_value = open_gamma_case(intrigue_enabled)
                collected_items.append((item, rarity, item_price,condition,float_value))
            else:
                print("Sorry,you don't have enough money")
           ...
           if collected_items:
               ...
               for i, (item, rarity, price, condition, float_value) in enumerate(collected_items):
                   ...
                   print(f'{i+1}.{item_color} {item} -{rarity} ({final_price}$){Style.RESET_ALL}'
               ...
               if items_to_sell:
                   ...
                   for idx in sorted(items_to_sell,reverse = True):
                       if 0 <= idx < len(collected_items):
                           ...
                           sold_items.append(f'{sold_item_color}{sold_item} ({final_price}$) '
            ...
            for item, rarity, item_price, condition, float_value in collected_items:
                ...
                print(f'{item_color}{item}{rarity}({final_price}$){Style.RESET_ALL}'
                      f' {Fore.LIGHTWHITE_EX}[{condition} : {float_value:.4f}] {Style.RESET_ALL}')
            break
        elif choice == "/":
...
def open_alpha_case(intrigue_enabled):
    ...
    print(f"Congratulations!You got: {item_color}{result}{Style.RESET_ALL}"
          f"{Fore.LIGHTWHITE_EX} [{condition} : {float_value:.4f}] {Style.RESET_ALL}")
    return result, rarities[result]['rarity'], item_price, condition, float_value
